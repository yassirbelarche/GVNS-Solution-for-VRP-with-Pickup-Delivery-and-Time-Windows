...
def or_opt_search(sub_tour, distance, travel_time, service_time, ready_time, due_time, K):
    ...
    for i in range(len(sub_tour)):
        [Node1, Node2, Node3, Imp] = or_opt_move(sub_tour[i], distance, travel_time, service_time, ready_time, due_time,K)
...
for file in file_names:
    file_name = os.path.join('/content/', file + '.csv')
    print(file_name)
    ...
    dist_matrix_raw = np.genfromtxt('/content/matdistfinale.csv',delimiter=',')
    time_matrix_raw = np.genfromtxt('/content/mattempsfinale.csv', delimiter=',')
    ...
    #  = [[0, 19, 23, 21, 18, 25, 0], [0, 22, 20, 24, 0], [0, 2, 5, 8, 7, 6, 3, 1, 4, 0], [0, 14, 12, 16, 15, 11, 9, 10, 13, 17, 0]]
...
#start
#1 1158489.0 4 2.817072868347168

    print('\n', min(NO_VEHICLE), np.mean(NO_VEHICLE), np.std(NO_VEHICLE))
    print(min(DIST), np.mean(DIST), np.std(DIST), np.mean(RUN_TIME))
    print("====================")

Solution = [[0, 15, 17, 16, 19, 18, 0], [0, 22, 12, 9, 8, 0], [0, 4, 3, 6, 5, 7, 2, 1, 0], [0, 20, 21, 23, 24, 25, 13, 14, 28, 26, 27, 0]]
